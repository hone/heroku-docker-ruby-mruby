...
APP_NAME=ENV["APP_NAME"] || "heroku-docker-ruby-util"
...
begin
  ...
  desc "compile binary"
  task :compile => [:all] do
    %W(#{MRUBY_ROOT}/build/host/bin/#{APP_NAME} #{MRUBY_ROOT}/build/x86_64-pc-linux-gnu/bin/#{APP_NAME}).each do |bin|
...
namespace :local do
  ...
  task :build do
    ...
    Dir.mktmpdir do |dir|
      Dir.chdir(dir) do
        FileUtils.cp("#{MRUBY_ROOT}/build/x86_64-pc-linux-gnu/bin/#{APP_NAME}", ".")
        FileUtils.cp("#{MRUBY_ROOT}/../docker-ruby/init.sh", ".")
        FileUtils.cp("#{MRUBY_ROOT}/../docker-ruby/Dockerfile", ".")
        sh "docker build -t hone/ruby ."
      end
    end
  end
